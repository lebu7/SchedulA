...
async function logSMS(phone, message, status, details) {
  try {
    ...
    if (message.includes("Confirmed") || message.includes("Received"))
...
export async function sendBookingConfirmation(
  ...
  const date = new Date(appointment.appointment_date).toLocaleString("en-KE", {
    dateStyle: "medium",
    timeStyle: "short",
  });
...
export async function sendCancellationNotice(
  ...
  if (cancelledBy === "provider")
    actorText = `${appointment.provider_name || "The provider"} has cancelled the appointment`;
  if (cancelledBy === "client")
    actorText = "You have cancelled the appointment";
      ...
      async (err, rows) => {
        ...
        for (const row of rows) {
          ...
          await sendAppointmentReminder(
            ...
            { business_name: row.business_name, name: row.provider_name },
          );
          db.run("UPDATE appointments SET reminder_sent = 1 WHERE id = ?", [
            row.id,
          ]);
        }
      },
