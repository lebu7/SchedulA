...
function BookingModal({ service, user, onClose, onBookingSuccess, isWalkIn = false }) {
  ...
  const [selectedDate, setSelectedDate] = useState(
    isWalkIn ? new Date().toISOString().split("T")[0] : ""
  );
  ...
  const handleDateChange = async (dateValue) => {
    ...
    try {
      const providerRes = await api.get(
        `/auth/public-profile/${serviceMeta.provider_id}`
      );
  ...
  const fetchAvailability = async () => {
    ...
    try {
      ...
      if (availabilityData.is_closed) {
        setError(
          `Provider is closed on this day: ${
            availabilityData.closed_reason || "Day off"
          }`
        );
      } else {
        generateTimeSlots(
  ...
  const generateTimeSlots = (openTime, closeTime, bookedRanges, myBookings) => {
    ...
    while (current < end) {
      ...
      const isBookedByMe =
        ...
        !isWalkIn &&
        myBookings.some((myAppt) => {
          const myTime = new Date(myAppt.appointment_date)
            .toTimeString()
            .slice(0, 5);
  ...
  useEffect(() => {
    const addonTotal = selectedAddons.reduce(
      (sum, a) => sum + Number(a.price ?? a.additional_price ?? 0),
      0
    );
  ...
  const disableClose = processingPayment || processingBooking;
  ...
  return (
    ...
    >
      ...
      >
        {/* ✅ (18) Header class hook for colored headers in CSS */}
        <div className={`modal-header booking-modal-header ${isWalkIn ? "walkin" : ""}`}>
          <div>
            <h3>{isWalkIn ? "Record Walk-In" : `Book ${serviceMeta?.name}`}</h3>
            <span className="subtitle">
              {isWalkIn ? "Blocks 1 slot & records revenue" : `with ${serviceMeta?.provider_name}`}
            </span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="booking-form">
            ...
            {step === 1 ? (
              <div className="step-1">
                <div className="form-group">
                  ...
                  {isWalkIn && (
                    <small style={{ color: "#64748b", fontSize: "11px" }}>
                      Walk-ins are recorded for today.
                    </small>
                  )}
                </div>
                ...
                {isWalkIn && (
                  <div
                    className="walk-in-payment-section"
                    style={{
                      background: "#f8fafc",
                      padding: "15px",
                      borderRadius: "8px",
              ...
              <div className="step-2">
                ...
                <div className="two-column-layout">
                  <div className="column left-col">
                    ...
                    <div className="form-group relative-container">
                      <div className="addon-dropdown" ref={addonDropdownRef}>
                        <button
                          type="button"
                          className={`addon-dropdown-toggle ${
                            showAddonDropdown ? "active" : ""
                          }`}
                                ...
                                {addons.map((addon) => (
                                  <label key={addon.id} className="addon-item">
                                    ...
                                    <span className="addon-price">
                                      +KES {addon.price ?? addon.additional_price ?? 0}
                                    </span>
                                  </label>
                                ))}
                              </div>
                    ...
                    <div className="payment-options-box">
                      ...
                      >
                        ...
                        <div className="option-details">
                          <span className="option-title">Deposit</span>
                ...
                <div className="booking-policy-container">
                  <div className="policy-text">
                    ...
                    <em>before</em> the appointment time. The deposit secures your spot;{" "}
                    <strong>no-shows are non-refundable</strong>.
                  </div>
                </div>
                      ...
                      <PaystackButton
                        ...
                        {...{
                          ...
                          text: processingPayment
                            ? "Processing..."
                            : `Pay KES ${depositKES.toLocaleString()}`,
                        }}
                        disabled={payDisabled}
                      />
                    )}

                    {fullConfig.publicKey && paymentOption === "full" && (
                      <PaystackButton
                        className="btn btn-primary btn-block"
                        {...{
                          ...fullConfig,
                    ...
                    {paymentOption === "custom" && (
                      <PaystackButton
                        ...
                        {...{
                          ...
                          text: processingPayment
                            ? "Processing..."
                            : `Pay KES ${computeSelectedAmountKES().toLocaleString()}`,
                        }}
                        disabled={payDisabled}
                      />
                    )}
